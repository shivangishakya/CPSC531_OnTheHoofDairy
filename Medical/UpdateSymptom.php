...
input[type= submit]{ background-color: orange;
     ...
     ?>
       <tr>
         ...
         <!-- Checkbox -->
         <td><input type='checkbox' name='update[]' value='<?= $SymptomID ?>' ></td>

         <td><?= $SymptomID ?></td>
         <td><input type='text' name='SymptomName_<?= $SymptomID ?>' value='<?= $SymptomName ?>' ></td>
       </tr>
     <?php
...
if(isset($_POST['update'])){
  foreach($_POST['update'] as $updateid){
    ...
    $SymptomName = $_POST['SymptomName_'.$updateid];

    $updateUser = "UPDATE Symptoms SET
                    SymptomName='".$SymptomName."'
                    WHERE SymptomID=".$updateid;   
    }
