...
input[type= submit]{ background-color: orange;
     ...
     ?>
       <tr>
         ...
         <td><?= $ID ?></td>
         <td><input type='number' name='PurchaseID_<?= $ID ?>' value='<?= $PurchaseID ?>' ></td>
         <td><input type='number' name='ItemID_<?= $ID ?>' value='<?= $ItemID ?>' ></td>
...
if(isset($_POST['update'])){
  ...
  foreach($_POST['update'] as $updateid){
    $sum = 0;
    $PurchaseID = $_POST['PurchaseID_'.$updateid];   
    $ItemID = $_POST['ItemID_'.$updateid];
    ...
    if ($result->num_rows > 0)
    {
        while($array=mysqli_fetch_row($result))
        {
            if ($ItemID === $array[0])
            { 
                $sum = $sum + $array[3]*$Quantity;
            }
        }
    } 
      
    $updateUser = "UPDATE ItemPurchase SET 
...
if(isset($_POST['FeedP'])){
    header("Location: UpdatePurchase.php");
